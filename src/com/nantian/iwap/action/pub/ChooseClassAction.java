...
public class ChooseClassAction extends CRUDAction {
	private static Logger logger = Logger.getLogger(ChooseClassAction.class);

	@Override
	protected int query(DTBHelper dtbHelper) {
		String menuName = "%"+dtbHelper.getStringValue("menuName")+"%";
		String sql = "select fid,ac_node_type,ac_parent,ac_name,ac_code,ac_unit,ffinancecategory,fyslbname,ac_bp,ac_description from ASSET_CLASSIFICATION where 1=1 "
		...
		try {
			...
			for (DataObject tmp : resultList) {
				zNodes += String.format("{id:'%s', ac_node_type:'%s', PID:'%s', ac_name:'%s',ac_code:%s,ac_unit:'%s',ffinancecategory:'%s',fyslbname:'%s',ac_bp:'%s',ac_description:'%s'},",
						...
						tmp.getValue("fyslbname"), tmp.getValue("ac_bp"), tmp.getValue("ac_description")
						);
			}
			if (zNodes.length() > 0) {
