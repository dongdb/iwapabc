...
/**
 * ClassName: RepairTpAction <br/>
 * Function: 获取维修类型菜单<br/>
 ...
 */
public class RepairTpAction extends TransactionBizAction {

	private static Logger log = Logger.getLogger(RepairTpAction.class);
	...
	protected int query(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			log.error("维修类型查询出错", e);
			dtbHelper.setError("repairtp-err-qry", "[维修类型查询出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int add(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			String sqlStr = "select FNAME from OA_PUB_BASECODE where "
						  + "FSCOPE ='资产维修类型' and FNAME = ?";
			DataObject result = dbBean.executeSingleQuery(sqlStr, FNAME);
			if (result != null) {
				dbBean.executeRollBack();
				log.warn("维修类型新增出错：该维修类型已存在!");
				dtbHelper.setError("repairtp-err-add-001", "[维修类型新增出错]该维修类型已存在!");
				return flag;
			}
			...
			dbBean.executeUpdate(sqlStr, FID, 0,"资产维修类型",FNAME,FSEQUENCE,FDESCRIPTION,
					FUSESTATUS,FUSESTATUSNAME,ORGID,PSNID,PSNNM,time,time);
			dbBean.executeCommit();
			flag = 1;
		} catch (Exception e) {
			log.error("维修类型新增出错", e);
			dtbHelper.setError("repairtp-err-add-002", "[维修类型新增出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
	...
	protected int save(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			log.error("维修类型保存出错", e);
			dtbHelper.setError("repairtp-err-sv", "[维修类型保存出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int remove(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			int u_cnt = 0;
			int s_cnt = 0;
			for (String repair : reparr) {
				if (repair == null || "".equals(repair.trim())) {
					continue;
				}
				
				u_cnt++;
				int i = dbBean.executeUpdate("delete from OA_PUB_BASECODE where FID = ?", repair);
				if (i == 1) {
					s_cnt++;
				}
			}
			dbBean.executeCommit();
			if (u_cnt != s_cnt) {
				log.warn("维修类型删除出错:删除失败" + (u_cnt - s_cnt) + "条");
				dtbHelper.setError("repairtp-err-rm-001", "[维修类型删除出错]删除失败" + (u_cnt - s_cnt) + "条");
			} else {
				flag = 1;
			}
		} catch (Exception e) {
			log.error("维修类型删除出错", e);
			dtbHelper.setError("repairtp-err-rm-002", "[维修类型删除出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
