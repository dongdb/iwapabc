 ...
 */
public class DealTpAction extends TransactionBizAction {

	private static Logger log = Logger.getLogger(DealTpAction.class);
	...
	protected int query(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			log.error("处置方式查询出错", e);
			dtbHelper.setError("dealtp-err-qry", "[处置方式查询出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int add(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			String sqlStr = "select FNAME from OA_PUB_BASECODE where "
						  + "FSCOPE ='资产处置类型' and FNAME = ?";
			DataObject result = dbBean.executeSingleQuery(sqlStr, FNAME);
			if (result != null) {
				dbBean.executeRollBack();
				log.warn("处置方式新增出错：该处置方式已存在!");
				dtbHelper.setError("dealtp-err-add-001", "[处置方式新增出错]该处置方式已存在!");
				return flag;
			}
			...
			dbBean.executeUpdate(sqlStr, FID, 0,"资产处置类型",FNAME,FSEQUENCE,FDESCRIPTION,
					FUSESTATUS,FUSESTATUSNAME,ORGID,PSNID,PSNNM,time,time);
			dbBean.executeCommit();
			flag = 1;
		} catch (Exception e) {
			log.error("处置方式新增出错", e);
			dtbHelper.setError("dealtp-err-add-002", "[处置方式新增出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
	...
	protected int save(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			log.error("处置方式保存出错", e);
			dtbHelper.setError("dealtp-err-sv", "[处置方式保存出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int remove(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			int u_cnt = 0;
			int s_cnt = 0;
			for (String deal : reparr) {
				if (deal == null || "".equals(deal.trim())) {
					continue;
				}
				
				u_cnt++;
				int i = dbBean.executeUpdate("delete from OA_PUB_BASECODE where FID = ?", deal);
				if (i == 1) {
					s_cnt++;
				}
			}
			dbBean.executeCommit();
			if (u_cnt != s_cnt) {
				log.warn("处置方式删除出错:删除失败" + (u_cnt - s_cnt) + "条");
				dtbHelper.setError("dealtp-err-rm-001", "[处置方式删除出错]删除失败" + (u_cnt - s_cnt) + "条");
			} else {
				flag = 1;
			}
		} catch (Exception e) {
			log.error("处置方式删除出错", e);
			dtbHelper.setError("dealtp-err-rm-002", "[处置方式删除出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
