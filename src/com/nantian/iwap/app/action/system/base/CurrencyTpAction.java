...
/**
 * ClassName: CurrencyTpAction <br/>
 * Function: 获取货币类型菜单<br/>
 ...
 */
public class CurrencyTpAction extends TransactionBizAction {

	private static Logger log = Logger.getLogger(CurrencyTpAction.class);
	...
	protected int query(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			String sqlStr = "select FID,VERSION,FNAME,FCODE,FDESCRIPTION,FUSESTATUSNAME  "
			...
			log.error("货币类型查询出错", e);
			dtbHelper.setError("currencytp-err-qry", "[货币类型查询出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int add(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			String sqlStr = "select FNAME from OA_PUB_BASECODE where "
						  + "FSCOPE ='货币类型' and FNAME = ?";
			DataObject result = dbBean.executeSingleQuery(sqlStr, FNAME);
			if (result != null) {
				dbBean.executeRollBack();
				log.warn("货币类型新增出错：该货币类型已存在!");
				dtbHelper.setError("currencytp-err-add-001", "[货币类型新增出错]该货币类型已存在!");
				return flag;
			}
			...
			sqlStr = "insert into OA_PUB_BASECODE "
					+ "(FID,VERSION,FSCOPE,FNAME,FSEQUENCE,FCODE,FDESCRIPTION,FUSESTATUS,FUSESTATUSNAME,"
					+ "FCREATEOGNID,FCREATEPSNID,FCREATEPSNNAME,FCREATETIME,FUPDATETIME) "
					+ "values (?,?,?,?,?,?,?,?,?,?,?,?,to_date(?,'yyyy-MM-dd HH24:mi:ss'),to_date(?,'yyyy-MM-dd HH24:mi:ss'))";
			dbBean.executeUpdate(sqlStr, FID, 0,"货币类型",FNAME,FSEQUENCE,FCODE,FDESCRIPTION,
					FUSESTATUS,FUSESTATUSNAME,ORGID,PSNID,PSNNM,time,time);
			dbBean.executeCommit();
			flag = 1;
		} catch (Exception e) {
			log.error("货币类型新增出错", e);
			dtbHelper.setError("currencytp-err-add-002", "[货币类型新增出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
	...
	protected int save(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			String sqlStr = "UPDATE OA_PUB_BASECODE set "
						  + "FNAME=?,FCODE=?,FDESCRIPTION=?,VERSION=?,FUSESTATUS=?,FUSESTATUSNAME=?, "
						  + "FUPDATEPSNID=?,FUPDATEPSNNAME=?,FUPDATETIME=to_date(?,'yyyy-MM-dd HH24:mi:ss')"
						  + " where FID=?";
			dbBean.executeUpdate(sqlStr, FNAME, FCODE, FDESCRIPTION, VERSION, FUSESTATUS,
								 FUSESTATUSNAME,PSNID,PSNNM,time,FID);
			dbBean.executeCommit();
			flag = 1;
		} catch (Exception e) {
			log.error("货币类型保存出错", e);
			dtbHelper.setError("currencytp-err-sv", "[货币类型保存出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int remove(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			int u_cnt = 0;
			int s_cnt = 0;
			for (String currency : reparr) {
				if (currency == null || "".equals(currency.trim())) {
					continue;
				}
				
				u_cnt++;
				int i = dbBean.executeUpdate("delete from OA_PUB_BASECODE where FID = ?", currency);
				if (i == 1) {
					s_cnt++;
				}
			}
			dbBean.executeCommit();
			if (u_cnt != s_cnt) {
				log.warn("货币类型删除出错:删除失败" + (u_cnt - s_cnt) + "条");
				dtbHelper.setError("currencytp-err-rm-001", "[货币类型删除出错]删除失败" + (u_cnt - s_cnt) + "条");
			} else {
				flag = 1;
			}
		} catch (Exception e) {
			log.error("货币类型删除出错", e);
			dtbHelper.setError("currencytp-err-rm-002", "[货币类型删除出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
