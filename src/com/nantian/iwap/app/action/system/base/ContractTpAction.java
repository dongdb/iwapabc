 ...
 */
public class ContractTpAction extends TransactionBizAction {

	private static Logger log = Logger.getLogger(ContractTpAction.class);
	...
	protected int query(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			log.error("合同类型查询出错", e);
			dtbHelper.setError("contracttp-err-qry", "[合同类型查询出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int add(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			String sqlStr = "select FNAME from OA_PUB_BASECODE where "
						  + "FSCOPE ='合同类型' and FNAME = ?";
			DataObject result = dbBean.executeSingleQuery(sqlStr, FNAME);
			if (result != null) {
				dbBean.executeRollBack();
				log.warn("合同类型新增出错：该合同类型已存在!");
				dtbHelper.setError("contracttp-err-add-001", "[合同类型新增出错]该合同类型已存在!");
				return flag;
			}
			...
			dbBean.executeUpdate(sqlStr, FID, 0,"合同类型",FNAME,FSEQUENCE,FDESCRIPTION,
					FUSESTATUS,FUSESTATUSNAME,ORGID,PSNID,PSNNM,time,time);
			dbBean.executeCommit();
			flag = 1;
		} catch (Exception e) {
			log.error("合同类型新增出错", e);
			dtbHelper.setError("contracttp-err-add-002", "[合同类型新增出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
	...
	protected int save(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			log.error("合同类型保存出错", e);
			dtbHelper.setError("contracttp-err-sv", "[合同类型保存出错]" + e.getMessage());
		}
		return flag;
	}
	...
	protected int remove(DTBHelper dtbHelper) throws BizActionException {
		...
		try {
			...
			int u_cnt = 0;
			int s_cnt = 0;
			for (String contract : reparr) {
				if (contract == null || "".equals(contract.trim())) {
					continue;
				}
				
				u_cnt++;
				int i = dbBean.executeUpdate("delete from OA_PUB_BASECODE where FID = ?", contract);
				if (i == 1) {
					s_cnt++;
				}
			}
			dbBean.executeCommit();
			if (u_cnt != s_cnt) {
				log.warn("合同类型删除出错:删除失败" + (u_cnt - s_cnt) + "条");
				dtbHelper.setError("contracttp-err-rm-001", "[合同类型删除出错]删除失败" + (u_cnt - s_cnt) + "条");
			} else {
				flag = 1;
			}
		} catch (Exception e) {
			log.error("合同类型删除出错", e);
			dtbHelper.setError("contracttp-err-rm-002", "[合同类型删除出错]" + e.getMessage());
			try {
				if (dbBean != null) {
					dbBean.executeRollBack();
				}
