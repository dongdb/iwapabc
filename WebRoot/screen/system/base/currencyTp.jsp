	...
	<div id="divDialog" class="divDialog">
		...
		<div class="dialog" id="myModal" style="width: 500px;">
			<div class="dialog-header">
				...
				<h4 class="dialog-title">新增货币类型</h4>
			</div>
				...
				<form method="post" onsubmit="return false" id="dialogarea">
					...
					<div class="col-md-12">
						<div class="inputbox pr" data-iwap="tooltipdiv"
							data-iwap-empty="false" data-iwap-tooltext="货币类型名称不能为空">
						...
						<div class="inputbox">
							<span>简称:</span> <input name="FCODE" type="text"
								data-iwap-xtype="TextField" id="FCODE" class="input_text">
						</div>
						<div class="inputbox">
	...
	<div class="table_box">
		...
		<table id="iwapGrid" class="mygrid table table-bordered table-striped table-hover"
			...
			<tr>
				...
				<th data-grid-name="FCODE" class="tl">简称</th>
...
$(document).ready(function() {
	parent.document.getElementById("title").value=document.title;
		...
		/*查询表格初始化  设置默认查询条件*/
		var fData={'actionId':'doBiz','start':'0','limit':'10','txcode':'currencyTp'};
...
function doSave(){
	...
	var extParam={'option':actionType,'txcode':'currencyTp','actionId':'doBiz'};
...
function del(){
	...
	var param={'option':"remove",'txcode':"currencyTp",'repids': iwapGrid.getCheckValues(),'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
...
function delOne(obj){
	...
	var param={'option':"remove",'txcode':"currencyTp",'repids': iwapGrid.getCurrentRow()['FID'],'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
		 	return alert("删除失败:"+rs['header']['msg']);
		 }else{
...
function stop(){
	...
	if (iwapGrid.getCurrentRow()["FUSESTATUSNAME"] == "启用"){
		var param={'option':"stop",'txcode':"currencyTp",'list':list,'actionId':"doBiz",'FID':FID};
...
function start(){
	...
	if (iwapGrid.getCurrentRow()["FUSESTATUSNAME"] == "启用"){
		...
		var param={'option':"restart",'txcode':"currencyTp",'list':list,'actionId':"doBiz",'FID':FID};
		$.IWAP.iwapRequest("iwap.ctrl",param,function(data){
			 if (data.body.ERROR) {
