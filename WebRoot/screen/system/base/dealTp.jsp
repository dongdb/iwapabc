	...
	<div id="divDialog" class="divDialog">
		...
		<div class="dialog" id="myModal" style="width: 500px;">
			<div class="dialog-header">
				...
				<h4 class="dialog-title">新增处置方式</h4>
			</div>
				...
				<form method="post" onsubmit="return false" id="dialogarea">
					...
					<div class="col-md-12">
						<div class="inputbox pr" data-iwap="tooltipdiv"
							data-iwap-empty="false" data-iwap-tooltext="处置方式名称不能为空">
...
$(document).ready(function() {
	parent.document.getElementById("title").value=document.title;
		...
		/*查询表格初始化  设置默认查询条件*/
		var fData={'actionId':'doBiz','start':'0','limit':'10','txcode':'dealTp'};
...
function doSave(){
	...
	var extParam={'option':actionType,'txcode':'dealTp','actionId':'doBiz'};
...
function del(){
	...
	var param={'option':"remove",'txcode':"dealTp",'repids': iwapGrid.getCheckValues(),'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
...
function delOne(obj){
	...
	var param={'option':"remove",'txcode':"dealTp",'repids': iwapGrid.getCurrentRow()['FID'],'actionId':"doBiz"};
	$.IWAP.iwapRequest("iwap.ctrl",param,function(rs){
		 if (rs['header']['msg']) {
		 	return alert("删除失败:"+rs['header']['msg']);
		 }else{
...
function stop(){
	...
	if (iwapGrid.getCurrentRow()["FUSESTATUSNAME"] == "启用"){
		var param={'option':"stop",'txcode':"dealTp",'list':list,'actionId':"doBiz",'FID':FID};
...
function start(){
	...
	if (iwapGrid.getCurrentRow()["FUSESTATUSNAME"] == "启用"){
		...
		var param={'option':"restart",'txcode':"dealTp",'list':list,'actionId':"doBiz",'FID':FID};
		$.IWAP.iwapRequest("iwap.ctrl",param,function(data){
			 if (data.body.ERROR) {
